...
export default function RessourceCategory() {
  const { slug } = useParams();
  const category = RESSOURCES_CATEGORIES.find((c) => c.slug === slug);
  const rootRef = useReveal('.res-anim', { stagger: 0.12 });

  if (!category) return <Navigate to="/ressources" replace />;

  const dark = category.dark;
  const others = RESSOURCES_CATEGORIES.filter((c) => c.slug !== slug);

  return (
    <section ref={rootRef} className={`pt-32 md:pt-40 pb-20 md:pb-28 px-6 md:px-10 ${dark ? 'bg-nuit' : ''}`}>
      <div className="max-w-content mx-auto">
        <div className="res-anim">
          <Breadcrumb dark={dark} crumbs={[{ label: 'Ressources', to: '/ressources' }, { label: category.label }]} />
        </div>
        <div className="res-anim flex items-center gap-3 mt-8 mb-5">
          <span className={`w-2.5 h-2.5 rounded-full ${category.dot} animate-pulse`} />
          <span className={`font-mono text-xs tracking-widest uppercase ${dark ? 'text-white/60' : 'text-nuit/50'}`}>
            Ressources
          </span>
        </div>
        <h1
          className={`res-anim font-heading font-bold text-4xl md:text-5xl max-w-2xl text-balance ${
            dark ? 'text-white' : 'text-nuit'
          }`}
        >
          {category.label}
        </h1>
        <p className={`res-anim leading-relaxed max-w-xl mt-5 mb-14 ${dark ? 'text-white/70' : 'text-nuit/70'}`}>
          {category.desc}
        </p>

        <div
          className={`res-anim relative overflow-hidden rounded-xl2 border ${category.border} ${
        ...
        >
          ...
          <p className={`relative text-sm max-w-sm leading-relaxed ${dark ? 'text-white/60' : 'text-nuit/60'}`}>
            Les {category.label.toLowerCase()} du SENAREC seront mis à disposition ici dès leur
            validation officielle.
          </p>
        </div>

        <div className="res-anim mt-16">
          <p className={`font-mono text-[11px] tracking-widest uppercase mb-4 ${dark ? 'text-white/40' : 'text-nuit/40'}`}>
            Autres ressources
          </p>
